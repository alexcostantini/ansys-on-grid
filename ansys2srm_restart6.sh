...
$clientSRM rm -e httpg://$closese:8444/ -w 1 -c 86400 -p -s srm://$closese:8444/ansys/$userfolder/$output".log"
echo "Done"
...
while [ "$try" -lt "4" ]; do
 ...
 if [ "$?" -ne "0" ]; then
  ...
  if [ "$try" -lt "4" ]; then
   ...
   $clientSRM pd -e httpg://darkstorm.cnaf.infn.it:8444 -s srm://darkstorm.cnaf.infn.it:8444/ansys/$userfolder/$output".log" -t $tokenlog
   echo ""
   exit 114
  fi
 else
...
tar -zcf $output.tar.gz --exclude='stdout.log' --exclude='stderr.log' --exclude='gridnfo.log'  --exclude='execute.bin' --exclude='wrapper.sh' --exclude='commands' --exclude='ansys.out' --exclude='ansys.err' --exclude='ansys2srm_restart5.sh' --exclude='ansyspid' --exclude='rstauto' *
#
